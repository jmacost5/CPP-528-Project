...
# store the year 2000 data
d4 <- data.frame( y=y1, treat=treat, post=0 )
# store the year 2010 data
d5 <- data.frame( y=y2, treat=treat, post=1 )

# stack the two time periods together
d6 <- rbind( d1, d2 )
d7 <- rbind( d4, d5 )

stargazer::stargazer(d6, d7,
                     type = S_TYPE,
                     digits = 2)
...
m1 <- lm( pov.rate ~ mhv_growth + post + mhv_growth*post, data=d )
m2 <- lm( pov.rate ~ mhv_growth + post + mhv_growth*post, data=d )
